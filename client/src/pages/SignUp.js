...
function Signup() {
  ...
  const handleSubmit = async(e) => {
    ...
    if(username && email && password){
      if(password){
        const fetchData = await fetch("http://localhost:8080/auth/register",{
          method: "POST",
          headers:{
  ...
  return (
    ...
    <div className='w-full max-w-sm bg-white m-auto flex items-center flex-col p-2'> 
        ...
        <form className='w-full py-3 flex flex-col' onSubmit={handleSubmit}>
          ...
          <div className='flex px-2 py-1 mt-1 bg-slate-200 mb-2 rounded outline-2 focus-within:outline-blue-300'> 
          <input type={ showConfirmPassword ? 'text':'password'} id='confirmppassword' name='confirmPassword' className='w-full bg-slate-200 border-none outline-none'  onChange={handleOnChange}/>
